...
public interface IAttendanceService
{
    ...
    // Same flow, spanning every class in every grade under one Stage (or, when
    // gradeId is set, every class/level under just that one grade). level ==
    // null means every level at once.
    Task<List<StageRosterEntryDto>> GetStageRosterAsync(Guid stageId, Guid academicYearId, StudentLevel? level, DateOnly date, Guid? gradeId = null);
