          ...
          {currentStep === 2 && (
            <div>
              ...
              <div style={{ marginTop: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.6rem', fontWeight: 600, fontSize: '0.9rem' }}>حالة المصاريف الإدارية للترم الحالي</label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', cursor: 'pointer' }}>
                    <input type="radio" name="feeStatus" checked={formData.feeStatus === 'paid'} onChange={() => handleFeeStatusChange('paid')} style={{ accentColor: 'var(--accent-gold)', width: '18px', height: '18px' }} />
                    <span style={{ fontSize: '0.9rem' }}>تم السداد</span>
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', cursor: 'pointer' }}>
                    <input type="radio" name="feeStatus" checked={formData.feeStatus === 'later'} onChange={() => handleFeeStatusChange('later')} style={{ accentColor: 'var(--c-blue)', width: '18px', height: '18px' }} />
                    <span style={{ fontSize: '0.9rem' }}>سداد لاحقاً</span>
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', cursor: 'pointer' }}>
                    <input type="radio" name="feeStatus" checked={formData.feeStatus === 'exempt'} onChange={() => handleFeeStatusChange('exempt')} style={{ accentColor: 'var(--accent-gold)', width: '18px', height: '18px' }} />
                    <span style={{ fontSize: '0.9rem' }}>إعفاء من الرسوم</span>
                  </label>
                </div>
