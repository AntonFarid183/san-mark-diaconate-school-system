...
// "download" requirement without extra tooling.
async function buildCardHtml(student) {
  ...
  return `
    <div class="card">
      <div class="card-header">مدرسة بي ثيؤريموس للألحان والتسبحة</div>
      <div class="card-body">
        <div class="photo">${photoUrl ? `<img src="${photoUrl}" />` : '<span>لا توجد صورة</span>'}</div>
        <div class="info">
          <div class="name">${student.fullName || ''}</div>
          <div>${student.stageName || ''}</div>
          <div>${student.gradeName || ''}</div>
          ${student.className ? `<div>فصل ${student.className}</div>` : ''}
          ${levelText ? `<div>${levelText}</div>` : ''}
        </div>
        <div class="qr">
          ${qrDataUrl ? `<img src="${qrDataUrl}" />` : ''}
          <div class="code">${student.studentCode || ''}</div>
        </div>
      </div>
    </div>`;
}
  ...
  w.document.write(`
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .card {
        width: ${CARD_WIDTH_MM}mm; height: ${CARD_HEIGHT_MM}mm; padding: 3mm;
        display: flex; flex-direction: column; gap: 1.5mm; page-break-after: always;
        color: #111;
      }
      .card-header { font-size: 2.6mm; font-weight: 700; text-align: center; border-bottom: 0.3mm solid #ccc; padding-bottom: 1mm; }
      .card-body { flex: 1; display: flex; gap: 2mm; align-items: center; }
      .photo { width: 16mm; height: 16mm; flex-shrink: 0; border-radius: 2mm; overflow: hidden; background: #eee; border: 0.2mm solid #ccc; display: flex; align-items: center; justify-content: center; font-size: 2mm; color: #999; }
      .photo img { width: 100%; height: 100%; object-fit: cover; }
      .info { flex: 1; min-width: 0; font-size: 2.1mm; display: flex; flex-direction: column; gap: 0.5mm; }
      .info .name { font-size: 2.8mm; font-weight: 700; }
      .qr { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; gap: 0.5mm; }
      .qr img { width: 16mm; height: 16mm; }
      .code { font-size: 1.8mm; direction: ltr; }
    </style></head>
